...
export class PostgresSieveRepository implements ISieveRepository {
  ...
  async getSellPriceCheck(id: string): Promise<SellPriceCheck | null> {
    const rows = await this.sql`SELECT * FROM sell_price_checks WHERE id = ${id} LIMIT 1`;
    if (!rows.length) return null;
    return this.mapSellCheckRow(rows[0]);
  }
  ...
  async listSellPriceChecks(params: ListFilterParams = {}): Promise<SellPriceCheck[]> {
    ...
    return rows.map((r) => this.mapSellCheckRow(r));
  }

  private mapSellCheckRow(r: Record<string, any>): SellPriceCheck {
    if (r.network !== "mainnet") {
      throw new SieveAppError("DATABASE_INTEGRITY_ERROR", "Non-Mainnet Sell check cannot be loaded by the Mainnet-only runtime");
    }
    return {
      id: r.id,
      network: "mainnet",
      ...
      asset: {
        ...
        supply: null,
        source: "PRESTOCKS",
        observedAt: new Date(r.reference_observed_at).toISOString(),
        network: "mainnet",
      },
      input: {
        requestedEconomicAmount: r.requested_economic_amount.toString(),
        actualEconomicAmount: r.actual_economic_amount.toString(),
        rawWalletInput: BigInt(r.raw_wallet_input),
        rawTransferFee: BigInt(r.raw_transfer_fee),
        rawRouteInput: BigInt(r.raw_route_input),
        decimals: Number(r.input_decimals),
        activeMultiplier: r.active_multiplier.toString(),
      },
      expectedUsdcProceedsRaw: BigInt(r.expected_usdc_proceeds_raw),
      expectedUsdcProceeds: r.expected_usdc_proceeds.toString(),
      priceImpactPct: r.price_impact_pct?.toString() ?? null,
      routeFingerprint: r.route_fingerprint,
      maxDiscountPct: (r.max_discount_bps / 100).toFixed(2),
      maxDiscountBps: r.max_discount_bps,
      decision: {
        status: r.decision,
        isExecutable: r.decision === "GOOD_TO_GO",
        referencePriceUsd: r.reference_price_usd.toString(),
        currentSellPriceUsd: r.current_sell_price_usd?.toString() ?? null,
        minimumSellPriceUsd: r.minimum_sell_price_usd.toString(),
        discountPct: r.discount_bps == null ? null : (r.discount_bps / 100).toFixed(2),
        maxDiscountPct: (r.max_discount_bps / 100).toFixed(2),
  ...
  async listSellTradeReceipts(params: ListFilterParams = {}): Promise<SellTradeReceipt[]> {
    ...
    if (params.wallet && params.network) {
      rows = await this.sql`
        SELECT * FROM sell_trade_receipts
        WHERE wallet = ${params.wallet} AND network = ${params.network}
        ORDER BY submitted_at DESC
        LIMIT ${limit} OFFSET ${offset}
      `;
    ...
    return rows.map((r) => this.mapSellReceipt(r));
  }
