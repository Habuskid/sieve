...
export class PostgresSieveRepository implements ISieveRepository {
  ...
  async saveSellPriceCheck(c: SellPriceCheck): Promise<void> {
    await this.sql`INSERT INTO sell_price_checks (id,network,wallet,target_name,target_symbol,target_mint,requested_economic_amount,actual_economic_amount,raw_wallet_input,raw_transfer_fee,raw_route_input,input_decimals,active_multiplier,expected_usdc_proceeds_raw,expected_usdc_proceeds,reference_price_usd,reference_observed_at,current_sell_price_usd,minimum_sell_price_usd,discount_bps,max_discount_bps,price_impact_pct,route_fingerprint,decision,source,practice_scenario_id,client_intent_version,created_at,expires_at) VALUES (${c.id},${c.network},${c.wallet},${c.asset.name},${c.asset.symbol},${c.asset.mint},${c.input.requestedEconomicAmount},${c.input.actualEconomicAmount},${c.input.rawWalletInput.toString()},${c.input.rawTransferFee.toString()},${c.input.rawRouteInput.toString()},${c.input.decimals},${c.input.activeMultiplier},${c.expectedUsdcProceedsRaw.toString()},${c.expectedUsdcProceeds},${c.decision.referencePriceUsd},${c.asset.observedAt},${c.decision.currentSellPriceUsd},${c.decision.minimumSellPriceUsd},${c.decision.discountBps},${c.maxDiscountBps},${c.priceImpactPct},${c.routeFingerprint},${c.decision.status},${c.source},${c.practiceScenarioId},${c.clientIntentVersion},${c.createdAt},${c.expiresAt}) ON CONFLICT (id) DO NOTHING`;
  }
  async getSellPriceCheck(id: string): Promise<SellPriceCheck|null> { const rows=await this.sql`SELECT * FROM sell_price_checks WHERE id=${id} LIMIT 1`; if(!rows.length)return null; const r=rows[0]; return { id:r.id,network:r.network,wallet:r.wallet,clientIntentVersion:r.client_intent_version,asset:{name:r.target_name,symbol:r.target_symbol,mint:r.target_mint,imageUrl:null,productUrl:null,referencePriceUsd:r.reference_price_usd.toString(),tokenPriceUsd:null,referenceValuationUsd:null,impliedValuationUsd:null,supply:null,source:r.source==="JUPITER"?"PRESTOCKS":"PRACTICE_FIXTURE",observedAt:new Date(r.reference_observed_at).toISOString(),network:r.network},input:{requestedEconomicAmount:r.requested_economic_amount.toString(),actualEconomicAmount:r.actual_economic_amount.toString(),rawWalletInput:BigInt(r.raw_wallet_input),rawTransferFee:BigInt(r.raw_transfer_fee),rawRouteInput:BigInt(r.raw_route_input),decimals:Number(r.input_decimals),activeMultiplier:r.active_multiplier.toString()},expectedUsdcProceedsRaw:BigInt(r.expected_usdc_proceeds_raw),expectedUsdcProceeds:r.expected_usdc_proceeds.toString(),priceImpactPct:r.price_impact_pct?.toString()??null,routeFingerprint:r.route_fingerprint,maxDiscountPct:(r.max_discount_bps/100).toFixed(2),maxDiscountBps:r.max_discount_bps,decision:{status:r.decision,isExecutable:r.decision==="GOOD_TO_GO",referencePriceUsd:r.reference_price_usd.toString(),currentSellPriceUsd:r.current_sell_price_usd?.toString()??null,minimumSellPriceUsd:r.minimum_sell_price_usd.toString(),discountPct:r.discount_bps==null?null:(r.discount_bps/100).toFixed(2),maxDiscountPct:(r.max_discount_bps/100).toFixed(2),discountBps:r.discount_bps,maxDiscountBps:r.max_discount_bps,differenceUsd:null,displayTitle:r.decision==="GOOD_TO_GO"?"The sell price is inside your limit.":"This sell is outside your limit.",displayMessage:""},practiceScenarioId:r.practice_scenario_id??null,source:r.source,createdAt:new Date(r.created_at).toISOString(),expiresAt:new Date(r.expires_at).toISOString() }; }
  ...
  private mapSellReceipt(r:Record<string,any>):SellTradeReceipt{return{id:r.id,side:"SELL",simulated:Boolean(r.simulated),checkId:r.check_id,buildIntentId:r.build_intent_id,wallet:r.wallet,network:r.network,signature:r.signature,internalExecutionId:r.internal_execution_id,status:r.status,targetSymbol:r.target_symbol,targetMint:r.target_mint,requestedEconomicAmount:r.requested_economic_amount.toString(),actualEconomicInput:r.actual_economic_input?.toString()??null,rawInput:r.raw_input,expectedUsdcProceeds:r.expected_usdc_proceeds.toString(),realizedUsdcProceeds:r.realized_usdc_proceeds?.toString()??null,referencePriceUsd:r.reference_price_usd.toString(),checkedSellPriceUsd:r.checked_sell_price_usd.toString(),minimumSellPriceUsd:r.minimum_sell_price_usd.toString(),maxDiscountBps:r.max_discount_bps,realizedDiscountBps:r.realized_discount_bps,submittedAt:new Date(r.submitted_at).toISOString(),confirmedAt:r.confirmed_at?new Date(r.confirmed_at).toISOString():null,failureCode:r.failure_code};}
