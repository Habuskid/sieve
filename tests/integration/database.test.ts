    ...
    const mockSql = ((strings: TemplateStringsArray, ...values: any[]) => {
      ...
      if (query.includes("INSERT INTO build_intents")) {
        savedBuildIntentRow = {
          ...
          transaction_hash: null, transaction_message_hash: values[10],
          last_valid_block_height: values[11],
          funding_asset: values[12],
          funding_amount_display: values[13],
          target_symbol: values[14],
          expected_target_amount: values[15],
          max_premium_pct: values[16],
          expires_at: values[17] ? new Date(values[17]) : new Date(),
          status: "READY_FOR_WALLET",
          created_at: new Date(),
          target_decimals: values[18],
          active_multiplier: values[19],
          chain_timestamp: values[20],
          epoch: values[21],
        };
        return Promise.resolve([]);
      }
